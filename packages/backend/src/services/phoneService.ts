...
export async function getPhonesByFilter(filters: {
  device?: string;
  imei?: string;
  color?: string;
  ...
  return await db
    ...
    .where(
      and(
        ...
        filters.imei
          ? eq(phoneTable.imei, filters.imei)
          : undefined,

        filters.color
          ? ilike(phoneTable.color, `%${filters.color}%`)
          : undefined,
        ...
        filters.category
          ? eq(phoneTable.category, filters.category)
          : undefined,

        filters.device_type
          ? eq(phoneTable.device_type, filters.device_type)
          : undefined,
        ...
        filters.sold !== undefined
          ? eq(phoneTable.sold, filters.sold === "true")
          : undefined,

        filters.is_deleted !== undefined
          ? eq(phoneTable.is_deleted, filters.is_deleted)
          : undefined
      )
    )
...
export const getAllPhones = async () => {
    return await db.select().from(phoneTable).orderBy(phoneTable.device_id);
}
